...
func (c *ClientsRepository) GetClients(filter model.ClientsFilterExceptStatus) ([]model.Client, error) {
	stmt, err := c.db.Prepare(`
		with arg(f_id, f_name, f_api_key) as (
		    select ?,?,?
		)
		...
		where 	(f_id is null or id = f_id) and
				...
				(f_api_key is null or api_key = f_api_key)
	`)
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	rows, err := stmt.Query(filter.Id, filter.Name, filter.ApiKey)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err = rows.Scan(&client.Id, &client.Name, &client.ApiKey)
		if err != nil {
			return nil, err
		}
...
func (c *ClientsRepository) Create(client model.Client) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(client.Id, client.Name, client.ApiKey)
	if err != nil {
		return err
	}
	return nil
...
func (c *ClientsRepository) Update(id model.ID, new model.Client) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(new.Id, new.Name, new.ApiKey, id)
	if err != nil {
		return err
	}
	return nil
